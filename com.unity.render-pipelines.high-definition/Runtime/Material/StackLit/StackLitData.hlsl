...
float4 SampleTexture2DScaleBias(TEXTURE2D_PARAM(textureName, samplerName), float textureNameUV, float textureNameUVLocal, float4 textureNameST, TextureUVMapping uvMapping)
{
...
float4 SampleTexture2DTriplanarScaleBias(TEXTURE2D_PARAM(textureName, samplerName), float textureNameUV, float textureNameUVLocal, float4 textureNameST, TextureUVMapping uvMapping)
{
#ifdef _USE_TRIPLANAR
    if (textureNameUV == TEXCOORD_INDEX_TRIPLANAR)
    {
        ...
        if (uvMapping.triplanarWeights[textureNameUVLocal].x > 0.0)
            val += uvMapping.triplanarWeights[textureNameUVLocal].x * SampleTexture2DScaleBias(TEXTURE2D_ARGS(textureName, samplerName), TEXCOORD_INDEX_PLANAR_YZ, textureNameUVLocal, textureNameST, uvMapping);
        if (uvMapping.triplanarWeights[textureNameUVLocal].y > 0.0)
            val += uvMapping.triplanarWeights[textureNameUVLocal].y * SampleTexture2DScaleBias(TEXTURE2D_ARGS(textureName, samplerName), TEXCOORD_INDEX_PLANAR_ZX, textureNameUVLocal, textureNameST, uvMapping);
        if (uvMapping.triplanarWeights[textureNameUVLocal].z > 0.0)
            val += uvMapping.triplanarWeights[textureNameUVLocal].z * SampleTexture2DScaleBias(TEXTURE2D_ARGS(textureName, samplerName), TEXCOORD_INDEX_PLANAR_XY, textureNameUVLocal, textureNameST, uvMapping);
    ...
    else
    {
        ...
        return SampleTexture2DScaleBias(TEXTURE2D_ARGS(textureName, samplerName), textureNameUV, textureNameUVLocal, textureNameST, uvMapping);
#ifdef _USE_TRIPLANAR
    }
#endif
}

float4 SampleTexture2DTriplanarNormalScaleBias(TEXTURE2D_PARAM(textureName, samplerName), float textureNameUV, float textureNameUVLocal, float4 textureNameST, float textureNameObjSpace, TextureUVMapping uvMapping, float scale)
{
    if (textureNameObjSpace)
    {
        ...
        float4 packedNormal = SampleTexture2DTriplanarScaleBias(TEXTURE2D_ARGS(textureName, samplerName), textureNameUV, textureNameUVLocal, textureNameST, uvMapping);
    ...
    else
    {
        ...
        if (textureNameUV == TEXCOORD_INDEX_TRIPLANAR)
        {
            ...
            if (uvMapping.triplanarWeights[textureNameUVLocal].x > 0.0)
            {
                float4 packedNormal = SampleTexture2DScaleBias(TEXTURE2D_ARGS(textureName, samplerName), TEXCOORD_INDEX_PLANAR_YZ, textureNameUVLocal, textureNameST, uvMapping);
            ...
            if (uvMapping.triplanarWeights[textureNameUVLocal].y > 0.0)
            {
                float4 packedNormal = SampleTexture2DScaleBias(TEXTURE2D_ARGS(textureName, samplerName), TEXCOORD_INDEX_PLANAR_ZX, textureNameUVLocal, textureNameST, uvMapping);
            ...
            if (uvMapping.triplanarWeights[textureNameUVLocal].z > 0.0)
            {
                float4 packedNormal = SampleTexture2DScaleBias(TEXTURE2D_ARGS(textureName, samplerName), TEXCOORD_INDEX_PLANAR_XY, textureNameUVLocal, textureNameST, uvMapping);
        ...
        float4 packedNormal = SampleTexture2DScaleBias(TEXTURE2D_ARGS(textureName, samplerName), textureNameUV, textureNameUVLocal, textureNameST, uvMapping);
...
//custom-begin: added option to define STACKLIT_SAMPLER_OVERRIDE to use single sampler
#ifdef STACKLIT_SAMPLER_OVERRIDE
#define SAMPLE_TEXTURE2D_SCALE_BIAS(name) SampleTexture2DTriplanarScaleBias(name, STACKLIT_SAMPLER_OVERRIDE, name##UV, name##UVLocal, name##_ST, uvMapping)
#define SAMPLE_TEXTURE2D_NORMAL_SCALE_BIAS(name, scale, objSpace) SampleTexture2DTriplanarNormalScaleBias(name, STACKLIT_SAMPLER_OVERRIDE, name##UV, name##UVLocal, name##_ST, objSpace, uvMapping, scale)
#else
...
//custom-end:
    ...
    surfaceData.anisotropyA = _Anisotropy; // In all cases we must multiply anisotropy with the map
    surfaceData.anisotropyB = _Anisotropy;
#else
   ...
   // Use overlay blend mode for detail abledo: (base < 0.5 ? (2.0 * base * blend) : (1.0 - 2.0 * (1.0 - base) * (1.0 - blend)))
    ...
    // AFAICT These don't exist as inputs in the discontinued StackLit shader, only in the StackLit shadergraph node
    surfaceData.specularColor = 0;
    surfaceData.bentNormalWS = surfaceData.normalWS;
    surfaceData.haziness = 0;
    surfaceData.hazeExtent = 0;
    surfaceData.hazyGlossMaxDielectricF0 = 0;

//custom-begin: added hooks for modifying SurfaceData and BuiltinData
#if defined(STACKLIT_SURFACE_DATA_MODIFIER)
    STACKLIT_SURFACE_DATA_MODIFIER(input, surfaceData);
#endif
//custom-end:
...
//custom-begin: added hooks for modifying SurfaceData and BuiltinData
#if defined(STACKLIT_BUILTIN_DATA_MODIFIER)
    STACKLIT_BUILTIN_DATA_MODIFIER(input, builtinData);
#endif
//custom-end:
