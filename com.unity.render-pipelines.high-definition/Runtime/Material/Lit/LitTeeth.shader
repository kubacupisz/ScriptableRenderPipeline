...
//custom-begin: trim variants, based Gawain_teeth.mat + Gawain_tongue.mat
...
//custom-begin: add surface shader for teeth
            #define LIT_SURFACE_DATA_MODIFIER TeethSurf

            #pragma shader_feature_local TEETH_ATTN_NONE TEETH_ATTN_LINEAR TEETH_ATTN_SKYPOLYGON
            #pragma shader_feature_local TEETH_DATA_FIXED_6 TEETH_DATA_VARIABLE_32
            //#define TEETH_ATTN_SKYPOLYGON 1
            //#define TEETH_DATA_FIXED_6 1

            #define SURFACE_GRADIENT
            #include "Packages/com.unity.render-pipelines.core/ShaderLibrary/Sampling/SampleUVMapping.hlsl"

            #if TEETH_DATA_FIXED_6
                #define NUM_VERTS 6
                #define MAX_VERTS NUM_VERTS
            #elif TEETH_DATA_VARIABLE_32
                #define NUM_VERTS _TeethVertexCount
                #define MAX_VERTS 32
            #endif
            #define LAST_VERT (NUM_VERTS - 1)

            uniform float4 _TeethParams;// x = lit potential min, y = lit potential max
            uniform float4 _TeethVertexData[MAX_VERTS];
            uniform int _TeethVertexCount;

            #include "Assets/Code/Gawain/Runtime/SphericalPolygon.hlsl"

            void TeethSurf(FragInputs input, inout SurfaceData surfaceData)
            {
                float3 positionWS = GetAbsolutePositionWS(input.positionRWS);
                float3 normalWS = surfaceData.normalWS;

                float litAttn = 1.0;
                float litAttnBiased = 1.0;
                {
                #if TEETH_ATTN_NONE

                    return;// no modification

                #elif TEETH_ATTN_LINEAR
                    ...
                    litAttn = saturate(dot(v / dd, positionWS - posBack));
                    litAttnBiased = _TeethParams.x + litAttn * (_TeethParams.y - _TeethParams.x);

                #elif TEETH_ATTN_SKYPOLYGON

                    float3 P[MAX_VERTS];
                    for (int i = 0; i != NUM_VERTS; i++)
                    {
                        //P[i] = normalize(mul(input.worldToTangent, _TeethVertexData[i].xyz - positionWS));
                    ...
                    litAttn = saturate(incidentSky / incidentMax);
                    litAttnBiased = _TeethParams.x + litAttn * (_TeethParams.y - _TeethParams.x);

                #endif
                }

                surfaceData.coatMask *= litAttn;
                surfaceData.baseColor.rgb *= litAttnBiased;
                surfaceData.ambientOcclusion *= litAttn;
                surfaceData.specularOcclusion *= litAttn;
            }
//custom-end:
