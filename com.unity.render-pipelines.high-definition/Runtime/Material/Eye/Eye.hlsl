...
EyeData GetEyeData(const in FragInputs input)
{
    float3 surfacePositionOS = TransformWorldToObject(input.positionRWS);
    float3 surfaceNormalWS = input.worldToTangent[2];
    float3 surfaceNormalOS = TransformWorldToObjectDir(surfaceNormalWS);
