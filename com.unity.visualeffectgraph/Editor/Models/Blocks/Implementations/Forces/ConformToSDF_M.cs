...
namespace UnityEditor.VFX.Block
{
    ...
    class ConformToSDF_M : VFXBlock
    {
        public override string name { get { return "Conform to SDF - Custom"; } }
        ...
        public class InputProperties
        {
            ...
            public float ousideDistMultiplier = 1.0f;
...
float outsideDist = ousideDistMultiplier * max(absPos.x,max(absPos.y,absPos.z));
float3 dir;
...
float3 sdfVel = velocity+(sign(deltaSpeed) * min(abs(deltaSpeed),deltaTime * lerp(stickForce,attractionForce,ratio)) * dir / mass);

//float3 sdfVel = sign(deltaSpeed) * min(abs(deltaSpeed),deltaTime * lerp(stickForce,attractionForce,ratio)) * dir / mass;
velocity = lerp(inputVelocity, sdfVel, lerpVelocity); ";
            }
        }
    }
}
